...
export function getPoint(event) {
 console.log(event)
  // Get the vertices
  let a = event.object.geometry.vertices[event.face.a];
  let b = event.object.geometry.vertices[event.face.b];
  let c = event.object.geometry.vertices[event.face.c];
...
export function convertoLatLng(point, radius) { //CONVERTO TO PACKAGE!
  radius = radius || 200;
  var latRads = Math.acos(point.y / radius);
  var lngRads = Math.atan2(point.z, point.x);
  var lat = (Math.PI / 2 - latRads) * (180 / Math.PI);
...
export var geodecoder = function (features) { //CONVERTO TO PACKAGE!
